...
void Tablero::imprimirPlano(){
	int SIZE=10, X=this->getColumnas(), Y=this->getFilas();
	createBMP(X, Y, SIZE);


	 for(int x=0;x<X;x++){
	 		for(int y=0;y<Y;y++){
	 			setBigPixel( x,  y,  SIZE , this->getCasillero(X, Y, 1)->getCelula()->getGenes().genAlfa, this->getCasillero(X, Y, 1)->getCelula()->getGenes().genGamma, this->getCasillero(X, Y, 1)->getCelula()->getGenes().genBeta);
	 		}
	 	}
}
